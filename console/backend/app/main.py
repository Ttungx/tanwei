"""
Tanwei Console - FastAPI Backend
统一管理员控制台后端服务，负责文件上传、静态资源托管，并作为代理转发给 edge-agent
"""
...
async def lifespan(_app: FastAPI):
    ...
    logger.info("Tanwei Console starting up...")
    logger.info(f"Edge Agent URL: {EDGE_AGENT_URL}")
    logger.info(f"Central Agent URL: {CENTRAL_AGENT_URL}")
    logger.info(f"Upload Directory: {UPLOAD_DIR}")
    yield


app = FastAPI(
    title="Tanwei Console",
    description="统一管理员控制台后端服务",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def process_detection(task_id: str, file_path: Path, original_size: int):
    ...
    try:
        ...
        # Send file to edge-agent
        async with aiofiles.open(file_path, "rb") as f:
            file_content = await f.read()

        # Simulate stages for demo (in real implementation, edge-agent handles this)
        ...
        # Call edge-agent detect API
        try:
            files = {"file": (file_path.name, file_content, "application/vnd.tcpdump.pcap")}
            logger.debug(f"[Task {task_id}] Calling edge-agent at {EDGE_AGENT_URL}")
            ...
            # Update task with edge-agent task_id for status polling
            agent_task_id = result.get("task_id")
            tasks[task_id]["agent_task_id"] = agent_task_id
            logger.info(f"[Task {task_id}] Edge-agent task created: {agent_task_id}")

            # Poll for status from edge-agent
            max_attempts = 120  # 2 minutes max
            for attempt in range(max_attempts):
                status_response = requests.get(
                    f"{EDGE_AGENT_URL}/api/status/{agent_task_id}",
                    timeout=10
                )
                ...
                # Map edge-agent stages to display stages
                stage_mapping = {
                ...
                if stage == "completed":
                    ...
                    result_response = requests.get(
                        f"{EDGE_AGENT_URL}/api/result/{agent_task_id}",
                        timeout=10
                    )
            ...
            # For demo purposes, generate mock result if edge-agent is not available
            logger.warning(f"[Task {task_id}] Edge-agent unavailable, using mock result: {e}")
...
@app.get("/api/edges")
def get_edges():
    return central_client.list_edges()


@app.get("/api/edges/{edge_id}/reports/latest")
def get_latest_edge_report(edge_id: str):
    return central_client.get_latest_report(edge_id)


@app.get("/api/edges/{edge_id}/analysis")
def get_latest_edge_analysis(edge_id: str):
    return central_client.get_latest_edge_analysis(edge_id)


@app.post("/api/edges/{edge_id}/analyze")
def analyze_single_edge(edge_id: str):
    return central_client.analyze_edge(edge_id)


@app.get("/api/network/analysis")
def get_latest_network_analysis():
    return central_client.get_latest_network_analysis()


@app.post("/api/network/analyze")
def analyze_full_network():
    return central_client.analyze_network()
...
async def serve_index():
    ...
    return {"message": "Tanwei Console API is running. Frontend not built yet."}
